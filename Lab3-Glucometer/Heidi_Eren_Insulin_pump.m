...
U_cap = 50;                 % 50 units in the reservoir
U_used = 0;                 % units delivered so far
rate_per_U = 120;           % μU·mL^-1·min per unit
...
for k = 2:N
    ...
    % total insulin (PID + bolus + basal)
B_basal = 0.5;                           % optional basal (μU/mL/min)
B_raw   = addIn_PID(k) + addIn_bolus + B_basal;   % desired rate

% add 50U reservoir with per-step rate limiting
U_left = U_cap - U_used;
if U_left <= 0
    B_total = 0;                          
else
    % max rate without exceeding U_cap
    B_step_max = (U_left * rate_per_U) / dt;   % μU/mL/min
    B_total = max(0, min(B_raw, B_step_max));  % clamp
end

% add up
U_used = U_used + (B_total * dt) / rate_per_U;

addIn_total(k) = B_total;

% ODE
dQ = - (beta * Q(k-1) + eta * Dk) / (gam^2 + Q(k-1)^2);
dG =  R0 - (E + S * I(k-1)) * G(k-1) + kq * Q(k-1);
dI =  Imax * (G(k-1)^2 / (alpha + G(k-1)^2)) - ki * I(k-1) + B_total;

Q(k) = Q(k-1) + dQ*dt;  G(k) = G(k-1) + dG*dt;  I(k) = I(k-1) + dI*dt;
